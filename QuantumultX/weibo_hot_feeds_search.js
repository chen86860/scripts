  ...
  for (let o of e.items)
    if ("feed" == o.category) isAd(o.data) || t.push(o);
      ...
      if (o.items.length > 0 && o.items[0].data?.itemid?.includes("search_input"))
        ...
        o.items.length > 0 &&
          (o.items = Object.values(o.items).filter((e) => "feed" == e.category || "card" == e.category)),
          t.push(o);
      }
    } else {
  ...
  for (let i of t) {
    ...
    if (i.mblog) {
      ...
      if ("bottom_mix_activity" == i.itemid) a = !1;
        ...
        let n = i.card_group;
        if (!n) continue;
        if (["guess_like_title", "cats_top_title", "chaohua_home_readpost_samecity_title"].indexOf(n[0].itemid) > -1)
          a = !1;
        else if (n.length > 1) {
          let d = [];
...
function removeSearch(e) {
  ...
  for (let o of e.items)
    "feed" == o.category
      ? isAd(o.data) || (o.data?.page_info?.video_limit && delete o.data.page_info.video_limit, t.push(o))
      : "group" == o.category
        ? ((o.items = o.items.filter((e) => e.data?.card_type === 17)), o.items.length > 0 && t.push(o))
        : checkSearchWindow(o) || t.push(o);
  return (
    (e.items = t),
    e.loadedInfo &&
    ((e.loadedInfo.searchBarContent = []), e.loadedInfo.headerBack && (e.loadedInfo.headerBack.channelStyleMap = {})),
...
function removePage(e) {
  return (
    ...
    (e.cards[0].card_group = e.cards[0].card_group.filter(
      (e) =>
        !(e?.actionlog?.ext?.includes("ads_word") || e?.itemid?.includes("t:51") || e?.itemid?.includes("ads_word"))
    )),
    e
  );
}
function removeCards(e) {
  ...
  for (let o of e.cards) {
    ...
    if (i && i.length > 0) {
      let a = [];
      for (let r of i)
        118 == r.card_type || isAd(r.mblog) || -1 != JSON.stringify(r).indexOf("res_from:ads") || a.push(r);
...
function removeHome(e) {
  ...
  for (let o of e.items) {
    ...
    if ("profileme_mine" == i)
      ...
      o.images?.length > 0 &&
        (o.images = o.images.filter((e) => "100505_-_chaohua" == e.itemId || "100505_-_recentlyuser" == e.itemId)),
        t.push(o);
    }
  }
  return (e.items = t), e;
...
function containerHandler(e) {
  mainConfig.removeInterestFriendInTopic &&
    ...
    (e.itemid.indexOf("infeed_may_interest_in") > -1
      ...
      : e.itemid.indexOf("infeed_friends_recommend") > -1 && (log("remove 超话好友关注"), (e.card_group = [])));
}
